...
>Holding years since phd constant,there is no strong evidence that supports this argument. on average, males receive about $7,923 more than females but that is only significant at the level when p = 0.0915 but not valid when p < 0.05.
...
>From the ANOVA F-test of comparing the two models, the model with that includes the years since phd is better than the model without it. This indicates that the model without the years since phd is a more biased model.
there seem to be a gender pay gap. However, this gap is not dependent on the gender but rather more on the years since PhD
